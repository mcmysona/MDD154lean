...
Démonstration :
  ...
  Comme x = 1 et x > 2 on obtient que 1 > 2
...
Attention : cet exemple de négation est incontournable mais l’exercice ci-dessous peut
prendre beaucoup de temps si votre stratégie n’est pas optimale. N’hésitez pas à passer
à l’exercice suivant sans conclure.
-/
...
Exemple "Utilisation d’une négation"
  ...
  Conclusion : x = 1
Démonstration :
  Comme x*(x - 1) = 0 on obtient que x = 0 ∨ x - 1 = 0
...
```
  Comme x*(x - 1) = 0 on obtient que x = 0 ∨ x - 1 = 0
  Comme x = 0 ∨ x - 1 = 0 et x ≠ 0 on obtient que x - 1 = 0
```
...
Démonstration :
  Supposons que y ≤ x
  Comme y ≤ x et x ≤ y on obtient que x = y
