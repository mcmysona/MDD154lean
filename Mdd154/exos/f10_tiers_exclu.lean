...
Démonstration :
  ...
  Supposons que  ¬ 0 ≤ x + y
  Comme ¬ 0 ≤ x + y on obtient que x + y < 0
  Calc
    |x + y| = -(x + y) puisque x + y < 0
    _       = -x + (-y) par calcul
    _       ≤ |x| + |y| puisque -x ≤ |x| et -y ≤ |y|
...
Exemple "Dans un triangle rectangle (non dégénéré) de côtés a, b et c,
  ...
  Conclusion : z < x + y
Démonstration :
  Supposons par l'absurde que z ≥ x + y
  Il suffit de montrer que x^2 + y^2 > x^2 + y^2
  Comme x > 0 et y > 0 on obtient que x + y > 0
  Comme x > 0 et y > 0 on obtient que x * y > 0
  Calc
...
/-
L’exercice suivant est bien plus court que l’exemple ci-dessus.
-/

Exercice "10.01 Nombres plus petits que tout nombre strictement positif"
  Données : (x : ℝ)
...
Exercice "10.02 Contraposition concrète"
  Données : (f : ℝ → ℝ)
...
-/
Exercice "10.03 Le principe de contraposition à partir du raisonnement par l’absurde."
  Données : (P Q : Énoncé)
  Hypothèses : (h : (¬ Q) ⇒ ¬ P)
  Conclusion : P ⇒ Q
Démonstration :
...
-/
Exercice "10.04 parité et élévation au carré"
  Données : (n : ℤ)
  Hypothèses :
...
définition de la négation. Il s’agit en fait d’un exercice de logique pur.
N’hésitez pas à passer cet exercice si vous êtes en retard sur le reste du groupe.
-/

Exercice "10.05 Négation d’un énoncé existentiel"
  Données : (n : ℤ)
  Hypothèses :
...
Démonstration :
  ...
  -- On calcule -- ou bien
  Supposons que 2 * -42 = 2 * 42
  Comme 2 * -42 = 2 * 42 on conclut que Faux
QED

Exercice "10.06 La fonction identité, x ↦ x, n'est pas majorée."
  Données :
  Hypothèses :
...
`Comme … on obtient que …` où la nouvelle hypothèse `h` peut être simplement
...
-/
Exercice "10.07 Le paradoxe du buveur"
...
Exercice "10.08 Nombres plus petits que tout nombre strictement positif"
  Données : (x : ℝ)
  Hypothèses :
...
Exercice "10.09 Caractérisation des fonctions strictement croissantes"
  Données : (f : ℝ → ℝ)
  Hypothèses :
